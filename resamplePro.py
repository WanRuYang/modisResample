...
for f in fs:
	...
	print com 
	subprocess.Popen('modis_convert.py -s "( 1 1 1 1 )" -o ' + outfile + ' -e 3310 -g 250 '+ f, shell=True).wait()
...
for t in tifs:
	print t
	clipCom1 = ' '.join(['gdalwarp','-cutline', inshp, \
		'-crop_to_cutline', t, \
		re.search('A\d+',t).group(0) + t[23:26] +'_ca.tif'])
	print clipCom1
